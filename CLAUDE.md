...
npm run audio    # rebuild src/data/audioBooks.ts by probing the recording host. Also committed
...
- **Every child of `.reader` names its own `grid-row`, and that is not decoration.** The rows used
  to be filled by source order against three tracks, which held only while the reader had exactly
  three children in exactly that order. Adding the listen strip between the bar and the body pushed
  the text into the third track and left the strip holding the `minmax(0, 1fr)`, which collapsed to
  **0px**, so 52px of controls drew straight over the first paragraph. Nothing threw. Naming the
...
the chapter through `speechSynthesis`, and every one went the same way: the voices a web page can
reach are not good enough to want, and on iOS the good Siri voices are not exposed to the browser at
all, so the ceiling was low and no amount of ranking, pitching or resetting moved it. The old note
here said what would actually change the answer, and it was right: **not better synthesis, a real
recording of this exact translation.** That is what this is.

- **The recording is the World English Bible read by David Williams**, made in 2000 and placed into
  the public domain by the narrator in 2016. It is the same translation the app ships, so the words
  on screen and the words in the ear are the same words, which no synthesised reading of a different
  edition could promise. `AUDIO_CREDIT` names him wherever it plays, the same reasoning that keeps
  the two text credits in the reader's footer: the licence does not ask, and printing somebody's
  work unattributed would be the wrong way round.
- **`src/data/audioBooks.ts` was generated by probing the server, not by reading its index**, and
  that is the whole reason it is right. The published index is wrong: it lists
  `22_Song_of_Soloman_01` and `25_Lamentations01` and both 404. Every entry was confirmed by asking
  for chapter one and the last chapter and getting 200 for both, 132 requests rather than 1,189. **A
  wrong filename does not throw**, it gives a play button that does nothing, which is why this was
  worth measuring instead of deriving.
- **The naming is three different shapes** because the set is volunteer work rather than a database
  export: `06_Joshua_01` for most books, `19_Psalm_150` for Psalms alone, `25_Lam1` for Lamentations
  alone with no separator and no padding, and a bare `65_Jude` for the one chapter books. All four
  are pinned by tests.
- **`recorded` is not the same as the app's chapter count, and conflating them would offer audio for
  a page nobody can be on.** This canon is Catholic, so Esther runs to 16 and Daniel to 14, and the
  recording stops at the Hebrew 10 and 12. A test fails if any entry claims more chapters than the
  text has.
- **Silence is a real answer that the screen has to give.** `audioUrl` returns null for the seven
  deuterocanonical books, for the Greek additions past the end of Esther and Daniel, and for 1 and 2
  Thessalonians, whose files are missing from the set entirely under every name the index publishes.
  The strip says so in a sentence rather than offering a dead control, because a button that does
  nothing reads as a bug and the reason here is a real one.
- **It is an `<audio>` element, which `sound.ts` forbids everywhere else, and the distinction is the
  rule rather than an exception to it.** The cues are Web Audio so a silenced phone stays silent: an
  interface chime talking over somebody who asked for quiet is the app being rude. This is the
  opposite case, somebody pressed play, so it belongs on the media channel and should behave like a
  podcast. Do not take this as licence to reach for `<audio>` for a chime.
- **The strip sits under the bar and never over the text**, because the point is to read along and
  anything covering the words would be a worse version of reading them. It stays on while you page
  through chapters, loads the new one and keeps playing, and goes off when the reader closes. It is
  deliberately not remembered across opens: sound starting by itself because of something you did
  days ago is startling, and the button is one tap.
- **`preload="none"`.** A chapter is about a megabyte and most opens are for reading, so nothing is
  fetched until somebody asks.
- **The weak point is that it streams from somebody else's server.** The files are not ours, not on
  our CDN, and not cached, since the service worker ignores cross-origin requests outright, so there
  is no offline listening and no second copy if that host goes away. `AUDIO_BASE` is one constant so
  re-hosting is a one line change. **Hosting it properly is the real fix, not a nicety**, and it is
  about 1.2 GB for the whole Bible.
- Two other sources were checked and rejected. `ebible.org/webaudio/` has open CORS and the best
  provenance but is badly incomplete, Mark stops at 7 and Psalms at 2. `ebible.org/eng-web/audio/`
  covers 48 of 66 books with chapter numbers spelled out in words. The archive.org copy of this same
  David Williams reading is tagged CC BY-NC-ND by its uploader and is New Testament only, so it is
  the worse copy of the thing we are already using.
...
- **The card presents itself the first time a book is opened, and never again.** Not "chapter 1",
  because someone who opens Psalms at 23 is still starting Psalms. **It used to also require that
  none of the book had been marked, and that was wrong in the flow people actually use**: chapters
  are marked from the journey screen far more often than from inside the reader, so by the time
  somebody opened a book for the first time it was frequently already "not new" and the card
  silently never appeared. The guard was written for readers who were mid-Genesis when this shipped,
  a problem that expired; what it costs now is that somebody deep into a book they had never opened
  here meets its introduction once. After that it stays a tap away behind the About pill. A
  chapter's note reveals itself once and is simply there after, so nobody watches the same lines
  rise twice. What has been seen is **device-local**, like the voice: seeing a card again on a
  second device is a small redundancy, not a harm, and it keeps the journal's whitelist untouched.
- **The sheet covers the whole reader, and is a child of `.reader` rather than of its body.** That
  is what makes it full screen: `.insightSheet` is `position: absolute; inset: 0` and resolves
  against the nearest positioned ancestor, so living in the body confined it to the text and left
  the bar and the footer framing it like a letterbox. It was deliberately the other way round once,
  so the book and chapter pickers stayed usable and somebody who opened the wrong book did not have
  to dismiss an introduction to fix it. That cost one tap and bought a card in a slot; an
  introduction to a book is worth the screen. The body stops scrolling while it is up. The note is held back until the
